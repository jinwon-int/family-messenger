...
root=Path(__file__).resolve().parents[1];web=root/'experiments/openmls-browser/web';work=Path(tempfile.mkdtemp(prefix='peer-protocol-',dir=root/'artifacts'));work.chmod(0o700)
allowed={x:'text/javascript' for x in ('peer-preparation-ui.js','peer-preparation-client.js','successor-handoff.js')};allowed.update({'peer-preparation.html':'text/html','peer-preparation.css':'text/css'})
class Handler(BaseHTTPRequestHandler):
 def do_GET(self):
  name=self.path.lstrip('/') or 'peer-preparation.html'
...
try:
 with sync_playwright() as pw:
  ...
  for mode in ('spawn-throw','clone-throw','missing-boot','bad-boot','duplicate-boot','wrong-id','extra-private','bad-key','bad-room','over-memory','failure','lock','broadcast','hide','late-lock'):
   ...
   p.select_option('#identity','alice');p.fill('#database',scope['database']);p.select_option('#action','prepare');p.locator('#request-file').set_input_files({'name':'request.json','mimeType':'application/json','buffer':json.dumps(doc).encode()});p.click('#request-load');expect(p.locator('#request-status')).to_contain_text('요청을 읽었습니다');p.fill('#confirmation',expected);p.fill('#password','generated-password-for-protocol-only-000');p.check('#consent');p.click('#run')
  ...
  p.goto(f'http://127.0.0.1:{server.server_port}/');p.wait_for_selector('#run')
  def parse(raw):return p.evaluate("async raw=>{try{let m=await import('/peer-preparation-client.js');return (await m.request(raw,'alice','family-mls-device-vault-synthetic-protocol')).sha256}catch{return null}}",raw)
  ...
  assert p.evaluate("async d=>{const m=await import('/peer-preparation-client.js'),r=await m.request(JSON.stringify(d),'alice',d.scopes[0].database);return Object.isFrozen(r.reservation)&&Object.isFrozen(r.reservation.context.peer)}",doc);checks['owned_immutable_reservation_before_private_worker']=True
