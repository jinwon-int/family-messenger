...
self.onmessage=async({data})=>{
 try{
  if(data.archive.length>200000)throw Error('fixture bound');
  ...
  for(const kind of ['wrong-sender','pending-as-history','damaged-provider','non-final']){
   ...
   const fresh=sodium.crypto_secretstream_xchacha20poly1305_init_push(key),cipher=sodium.crypto_secretstream_xchacha20poly1305_push(fresh.state,enc.encode(JSON.stringify(r)),aad,kind==='non-final'?sodium.crypto_secretstream_xchacha20poly1305_TAG_MESSAGE:sodium.crypto_secretstream_xchacha20poly1305_TAG_FINAL);
