// Disposable aggregate-custody caller. No password or private state is persisted.
...
window.stopWorker=stop;
window.spawn=name=>new Promise((resolve,reject)=>{
 if(workers.has(name))throw Error('duplicate');const worker=new Worker('./aggregate-vault-worker.js',{type:'module'});
