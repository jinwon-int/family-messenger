...
func TestWelcomeAssetClosedProfile(t *testing.T) {
	for _, change := range []string{"valid", "missing", "extra", "corrupt", "symlink", "directory", "version", "source", "url", "mime", "duplicate", "trailing"} {
		t.Run(change, func(t *testing.T) {
			...
			b, e := loadAssetProfile(files, raw, welcomePaths, 11)
			if change == "valid" {
				if e != nil || len(b.files) != 31 {
					t.Fatal(e)
				}
			} else if e == nil {
	...
	for _, pin := range [][]byte{encryptedManifest, vaultManifest, historyManifest, retiredHistoryManifest, aggregateManifest, aggregateHistoryManifest, successorManifest, candidateManifest, peerManifest, custodyManifest} {
		f, r := assetProfileFixture(t, pin)
		if _, e := loadAssetProfile(f, r, welcomePaths, 11); e == nil {
			t.Fatal("old profile substituted")
		}
	}
}

func TestWelcomeAssetAdmissionAndLegacy(t *testing.T) {
	testAssetAdmissionRoutingAndLegacyPreservation(t, welcomeManifest, welcomePaths, 11)
}

func TestCompiledWelcomeExplicitSelection(t *testing.T) {
	b, e := LoadWelcomeAssets()
	if compiledWelcomeAssets == nil {
		if e == nil || b != nil {
			t.Fatal("implicit assets")
		}
		return
	}
	if e != nil || b.version != 11 || len(b.files) != 31 {
		t.Fatal(e)
	}
