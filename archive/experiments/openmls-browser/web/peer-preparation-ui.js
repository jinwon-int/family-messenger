import {exact,scope,request,result} from './peer-preparation-client.js';
...
function controls(){const busy=!!current;for(const id of ['identity','database','action','password','consent','request-file','request-load','confirmation'])$(id).disabled=busy;
 ...
 $('run').disabled=busy||!valid||kind!=='prepare'||$('password').value.length<32||$('password').value.length>128||!$('consent').checked||(!accepted||$('confirmation').value!==accepted.sha256);$('export').disabled=busy||!publicResult;
}
...
async function reset(){stop();accepted=null;$('confirmation').value='';$('request-file').value='';$('request-details').textContent='';$('request-status').textContent='요청 파일을 직접 읽어 대조하세요.';$('request-section').hidden=$('action').value!=='prepare';
 controls();}
...
function unknown(s){if(finish(s,'unknown','저장 여부를 확인할 수 없습니다. 같은 보관함과 인계 요청으로 저장 상태를 확인하세요.')){$('uncertain').hidden=false;forget();}}
...
$('preparation-form').addEventListener('submit',async event=>{event.preventDefault();if(current||$('run').disabled||document.hidden)return;
 const identity=$('identity').value,database=$('database').value,kind=$('action').value,expected=structuredClone(accepted.reservation);
 ...
 let worker;try{worker=new Worker('/successor-peer-worker.js',{type:'module'});s.worker=worker;}catch{unknown(s);return;}
 worker.onerror=()=>unknown(s);worker.onmessage=async({data})=>{if(current!==s||s.generation!==generation)return;
 if(exact(data,'boot')&&data.boot===true&&!s.booted){s.booted=true;try{worker.postMessage({id:1,method:'successor',argument});}catch{unknown(s);}finally{argument.password=null;}return;}
 ...
 if(finish(s,'prepared','기존 키를 유지하며 인계 준비를 이 기기에 저장했습니다. 아직 새 대화 참여 권한은 없습니다.')){publicResult=value;$('public-result').textContent=JSON.stringify(value,null,2);controls();}}
 catch{unknown(s);}
 };
});
$('export').addEventListener('click',()=>{if(!publicResult||current||document.hidden)return;const url=URL.createObjectURL(new Blob([JSON.stringify(publicResult,null,2)+'\n'],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='peer-public-preparation.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
