import {exact,scope,request,result} from './custody-ceremony-client.js';
...
function controls(){const busy=!!current;for(const id of ['identity','role','database','action','password','consent','request-file','request-load','confirmation'])$(id).disabled=busy;
 const valid=scope($('identity').value,$('database').value,$('role').value),kind=$('action').value;
 $('run').disabled=busy||!valid||!['prepare','declare'].includes(kind)||$('password').value.length<32||$('password').value.length>128||!$('consent').checked||(!accepted||$('confirmation').value!==accepted.sha256);$('export').disabled=busy||!publicResult;
}
...
function unknown(s){if(finish(s,'unknown','저장 여부를 확인할 수 없습니다. 같은 보관함과 요청으로 확인하세요. 선언 확인은 같은 선언을 다시 보낼 수 있습니다.')){$('uncertain').hidden=false;forget();}}
...
$('preparation-form').addEventListener('submit',async event=>{event.preventDefault();if(current||$('run').disabled||document.hidden)return;
 ...
 const argument={identity,database,password:$('password').value,intent:{accepted:true,reservation:expected}};$('password').value='';$('consent').checked=false;forget();
 ...
 let worker;try{worker=new Worker(kind==='declare'?'/'+role+'-custody-worker.js':role==='peer'?'/successor-peer-worker.js':'/candidate-worker.js',{type:'module'});s.worker=worker;}catch{unknown(s);return;}
 worker.onerror=()=>unknown(s);worker.onmessage=async({data})=>{if(current!==s||s.generation!==generation)return;
 if(exact(data,'boot')&&data.boot===true&&!s.booted){s.booted=true;try{worker.postMessage({id:1,method:kind==='declare'?'declare':role==='peer'?'successor':'bind',argument});}catch{unknown(s);}finally{argument.password=null;}return;}
 ...
 s.receiving=true;worker.terminate();try{const value=await result(data.result,identity,database,role,kind,expected);if(current!==s||s.generation!==generation||document.hidden)return;
 if(finish(s,value.state,value.state==='prepared'?'이 기기에 인계 준비를 저장했습니다. 서버 선언은 별도 작업입니다.':value.state==='pair-declared'?'서버에서 양쪽 선언을 확인했습니다. 아직 대화 참여 권한은 없습니다.':'서버에서 내 선언을 확인했습니다. 상대 기기의 선언을 기다립니다.')){publicResult=value;$('public-result').textContent=JSON.stringify(value,null,2);controls();}}
 catch{unknown(s);}
 };
});
$('export').addEventListener('click',()=>{if(!publicResult||current||document.hidden)return;const url=URL.createObjectURL(new Blob([JSON.stringify(publicResult,null,2)+'\n'],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='custody-public-receipt.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
