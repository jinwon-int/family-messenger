...
import {confirmationHTTP,current,prefix} from './confirmation-wire.js';
export function confirmationWorker(wasm,store,normalize,role){
 ...
 self.onmessage=async({data})=>{
  ...
  try{
   ...
   if(dead||started||data.method!=='confirm')fail();started=true;const a=data.argument;
   ...
   const h=await current(store,expected,role);
   const observed=await confirmationHTTP(store,expected,h,role);
   await store.open(selected.database,selected.identity,selected.room);
   let local=await store.confirm(expected,h,observed);if(dead||local.committed!==true)fail();
   if(local.pending){
    const accepted=await confirmationHTTP(store,expected,h,role,local.pending);prefix(observed.records,accepted.records);
    local=await store.confirm(expected,h,accepted);
   }
