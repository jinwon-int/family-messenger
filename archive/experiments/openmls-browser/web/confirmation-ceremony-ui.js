import {exact,scope,request,result,describe} from './confirmation-ceremony-client.js';
...
function controls(){const busy=!!current;for(const id of ['identity','role','database','action','password','consent','request-file','request-load','confirmation'])$(id).disabled=busy;
 ...
 $('run').disabled=busy||!valid||!['confirm'].includes(kind)||$('password').value.length<32||$('password').value.length>128||!$('consent').checked||(!accepted||$('confirmation').value!==accepted.sha256);$('export').disabled=busy||!publicResult;
}
...
function unknown(s){if(finish(s,'unknown','저장 여부를 확인할 수 없습니다. 같은 보관함과 요청으로 확인하세요. 결과 확인은 저장된 같은 확인 정보를 다시 보낼 수 있습니다.')){$('uncertain').hidden=false;forget();}}
...
$('preparation-form').addEventListener('submit',async event=>{event.preventDefault();if(current||$('run').disabled||document.hidden)return;
 ...
 let worker;try{worker=new Worker('/'+role+'-confirmation-worker.js',{type:'module'});s.worker=worker;}catch{unknown(s);return;}
 worker.onerror=()=>unknown(s);worker.onmessage=async({data})=>{if(current!==s||s.generation!==generation)return;
 if(exact(data,'boot')&&data.boot===true&&!s.booted){s.booted=true;try{worker.postMessage({id:1,method:'confirm',argument});}catch{unknown(s);}finally{argument.password=null;}return;}
 ...
 if(finish(s,value.state,describe(value))){publicResult=value;$('public-result').textContent=JSON.stringify(value,null,2);controls();}}
 catch{unknown(s);}
 };
});
$('export').addEventListener('click',()=>{if(!publicResult||current||document.hidden)return;const url=URL.createObjectURL(new Blob([JSON.stringify(publicResult,null,2)+'\n'],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='confirmation-public-receipt.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
