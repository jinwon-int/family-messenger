...
function checksum(record) {
  ...
  const encoded = JSON.stringify(['family-mls-state-v1', record.version, record.identity,
...
function valid(record, verifyChecksum = true) {
  if (!exact(record, ['version', 'identity', 'revision', 'crypto', 'ledger', 'cursor', 'epoch', 'checksum']) || record.version !== 1 ||
      record.identity !== identity || typeof record.epoch !== 'string' || record.epoch.length > 20 ||
        ...
        try {
          ...
          if (operation === 'initialize') {
            if (exact(record, ['version', 'identity']) && record.version === 0 && record.identity === identity) {
              record = {version: 1, identity, revision: 1, crypto: staged_init(identity), ledger: [], cursor: 0, epoch: 'none', checksum: new Uint8Array(32)};
