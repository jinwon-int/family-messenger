...
public test identities, no human account authentication, and no browser UI yet.
...
Use HTTP API clients. Requests with an Origin or Sec-Fetch-Site header, or a Host
other than `127.0.0.1:<port>`, are rejected to prevent browser/DNS-rebinding access.
...
frontend, or fleet execution adapter yet.
...
Next: own browser UI/authenticated streaming contract, read state and media;
