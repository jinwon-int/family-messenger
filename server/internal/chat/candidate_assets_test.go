...
func TestCandidateAssetClosedProfile(t *testing.T) {
	for _, change := range []string{"valid", "missing", "extra", "corrupt", "symlink", "directory", "version", "source", "url", "mime", "duplicate", "trailing"} {
		t.Run(change, func(t *testing.T) {
			...
			b, e := loadAssetProfile(files, raw, candidatePaths, 8)
			if change == "valid" {
				if e != nil || len(b.files) != 15 {
					t.Fatal(e)
				}
			} else if e == nil {
	...
	for _, pin := range [][]byte{encryptedManifest, vaultManifest, historyManifest, retiredHistoryManifest, aggregateManifest, aggregateHistoryManifest, successorManifest} {
		f, r := assetProfileFixture(t, pin)
		if _, e := loadAssetProfile(f, r, candidatePaths, 8); e == nil {
			t.Fatal("old profile substituted")
		}
	}
}

func TestCandidateAssetAdmissionAndLegacy(t *testing.T) {
	testAssetAdmissionRoutingAndLegacyPreservation(t, candidateManifest, candidatePaths, 8)
}

func TestCompiledCandidateExplicitSelection(t *testing.T) {
	b, e := LoadCandidateAssets()
	if compiledCandidateAssets == nil {
		if e == nil || b != nil {
			t.Fatal("implicit assets")
		}
		return
	}
	if e != nil || b.version != 8 || len(b.files) != 15 {
		t.Fatal(e)
	}
