...
func Open(dir string) (_ *Store, err error) {
	...
	if version != 0 && version != 1 && version != 2 && version != 3 && version != 4 && version != 5 && version != 6 && version != 7 && version != 8 && version != 9 && version != 10 && version != 11 && version != 12 {
   ...
   COMMIT;`)
	...
	if version < 5 {
		if e = migrateSuccessorReservation(db, dir, version < 4); e != nil {
			return nil, e
		}
	}
	if version < 6 {
		if e = migrateSuccessorCustody(db, dir, version < 5); e != nil {
			return nil, e
		}
	}
	if version < 7 {
	...
	if version < 8 {
		if e = migrateSuccessorConfirmation(db, dir, version < 7); e != nil {
			return nil, e
		}
	}

	if version < 9 {
		if e = migrateSuccessorLease(db, dir, version < 8); e != nil {
			return nil, e
		}
	}

	if version < 10 {
		if e = migrateSuccessorRetirement(db, dir, version < 9); e != nil {
			return nil, e
		}
	}
	if version < 11 {
		if e = migrateSuccessorEnrollment(db, dir, version < 10); e != nil {
			return nil, e
		}
	}

	if version < 12 {
		if e = migrateSuccessorClosure(db, dir, version < 11); e != nil {
			return nil, e
		}
	}
