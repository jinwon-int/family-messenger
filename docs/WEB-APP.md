...
| 대화 목록 순서 | 최근 활동순으로 움직이지 않는 **고정 순서**(`src/room-order.js`). 기본은 가족방 → 개인방 → 다른 방, 같은 종류는 이름순. 목록 위 "↕ 순서"를 누르면 행마다 ▲▼가 붙고 PC에서는 행을 끌어 옮길 수 있다(편집 중에는 행을 눌러도 방이 열리지 않는다). 순서는 계정 account data `us.familychat.room_order`(`{rooms: [방 ID…]}`)에 저장돼 같은 계정의 다른 기기에도 적용되고, 가족마다 자기 순서를 가진다. 저장된 순서에 없는 새 방은 기본 순서로 맨 뒤에 붙는다 |
...
| 입력 중 표시 | 작성창에 입력하면 m.typing을 보낸다(30초 유지 창, 계속 입력하면 만료 전 재알림, 뜸하면 6초 뒤 해제, 전송·방 전환 시 즉시 해제). 다른 사람이 입력 중이면 단일 문구 "입력중.."을 맥박 애니메이션으로 방 머리글과 대화목록 이름 옆에 표시하고(`src/typing.js`, 문구는 strings.js 한 곳에서 관리) 연결 끊김·기기 전환 잔상은 45초 뒤 자동 정리한다 |
...
- 대화목록은 Page Up/Page Down으로 이전/다음 항목에 포커스를 옮긴다(roving focus, `src/keyboard.js` `listPageMove`). 휴대폰 세로(한 pane)에서는 Enter로 방을 열어 바로 작성창에 커서를 둔다. 2분할(목록|대화, 900px 이상 또는 휴대폰 가로 640~899)에서는 Page Up/Down이 오른쪽 대화를 바로 미리 보고, Enter가 작성창으로 커서를 보내며, 작성창에서 다시 Page Up/Down이면 목록 탐색으로 돌아간다(대화는 그대로). Home은 작성 중인 글을 유지하며 목록의 원래 방으로 돌아간다. 목록은 2초 갱신마다 갈아끊워지므로 포커스가 body로 떨어졌을 때만 마지막 위치로 되살린다(`restoreRoomListFocus`) — 시트가 열려 있으면 끓어쓰지 않는다.
...
- 기기 ID는 `localStorage`에 남기고, 같은 계정이 같은 브라우저에서 다시 로그인하면 `device_id`를
  ...
  올라가 다른 기기의 검증이 `m.key_mismatch`로 취소된다(2026-09-18). 검증 시트는 취소 사유(코드·취소한 쪽)를 보여준다.
  재사용 덕에 로그인마다 새 기기가 생겨 매번 기기 검증을 반복하고 가족봇의 기기 신뢰가 깨지는 일이 없다.
  같은 브라우저에서 두 탭이 동시에 로그인하면 먼저 연 탭의 토큰이 서버에서 무효화될 수 있다(한 기기 = 한 세션).
