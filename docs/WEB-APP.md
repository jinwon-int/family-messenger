...
| 입력 중 표시 | 작성창에 입력하면 m.typing을 보낸다(30초 유지 창, 계속 입력하면 만료 전 재알림, 뜸하면 6초 뒤 해제, 전송·방 전환 시 즉시 해제). 다른 사람이 입력 중이면 방 머리글 이름 옆에 "…님이 입력중입니다"를 표시하고(`src/typing.js`) 연결 끊김·기기 전환 잔상은 45초 뒤 자동 정리한다 |
