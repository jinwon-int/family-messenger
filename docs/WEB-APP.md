...
| 요구(1단계) | 이 슬라이스의 상태 |
|---|---|
| 한국어·휴대폰 우선 | 모든 문구는 `src/strings.js`에 한국어로 모아둔다. 휴대폰은 단일 pane(목록 ↔ 대화), 900px부터 2열(왼쪽 목록 340px · 오른쪽 대화). 셸이 화면 높이를 고정하고 목록 pane과 타임라인만 스크롤한다. 방을 열면 하단, 하단 근처면 자동 하단 유지, 위로 올려 둔 채 새 메시지가 오면 위치 보존 + "새 메시지 ↓" 배지 |
