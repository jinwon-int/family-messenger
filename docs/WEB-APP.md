...
- 대화목록은 ↑/↓로 항목 포커스를 옮기고 Enter로 연다(roving focus, `src/keyboard.js` `listArrowMove`). 목록은 2초 갱신마다 갈아끊워지므로 포커스가 body로 떨어졌을 때만 마지막 위치로 되살린다(`restoreRoomListFocus`) — 시트가 열려 있거나 글자를 쓰는 중에는 끓어쓰지 않는다.
