...
**상태:** 오너 결정(2026-09-13). 이 문서는 방향 결정이며 구현 완료·배포 완료를 뜻하지 않는다. **운영 전달·암호화는 이 문서(D)가 유지한다.**
**후속:** 2026-09-21 [결정 E](DECISION-2026-09-21-MATRIX-PLUS-NATIVE-E2EE.md)가 네이티브 MLS **개발 동결**과 「1~4단계 전에 전달층 자체화를 판단하지 않는다」를 해제한다. 운영 경로를 독자 스택으로 바꾸지는 않는다.
**대체하는 문서:** [OWN-SYSTEM.md](OWN-SYSTEM.md)의 "서버·전달·암호화를 직접 만든다" 부분,
[NATIVE-E2EE.md](NATIVE-E2EE.md)의 OpenMLS 통합 경로(운영 투입). 두 문서는 이력과 수용 기준으로 보존한다.
**유지하는 문서:** [VISION.md](VISION.md)(변경 없음), [ROADMAP.md](ROADMAP.md)(결정 E로 다시 개정).
...
- 운영 호스트 Matrix: 배포 5일, healthy, 매일 백업 성공, **메시지 0건**(활성 계정 2·방 2·기기 4).
...
평가 항목(둘 다 운영 호스트 격리 디렉터리에서, 운영 Synapse 무변경):
...
### 평가 결과 (2026-09-13, 운영 호스트 격리 디렉터리, 운영 무변경) — **Tuwunel 1.9.1 확정**
