...
- 가족방 허용 멤버는 `family_users`에 설정한 가족 사용자 ID + 운영자 + 봇 1개다. 판단 근거는 검증된 user ID뿐이며 방 이름·표시 이름은 쓰지 않는다. 허용 집합 밖 멤버가 방에 들어오면 **그 방의 응답만 중단**하고 `room_gate_blocked` 상태로 기록한다. 평문 폴백은 없으며, 멤버가 다시 허용 집합 안으로 돌아오면 응답을 재개한다. 중단된 동안 들어온 mention은 **큐에 쌓이지 않고 버려진다**(재개 후에도 답하지 않으므로 다시 mention해야 한다). 반대로 봇이 꺼져 있던 동안 쌓인 mention은 재시작 시 admission 창(24시간) 안의 것을 모두 한 번에 답한다(실측: 7건 연속 응답). 오래 꺼져 있었다면 재시작 전 이 점을 가족에게 알린다.
