...
def parse_manifest(data, vault=False, history=False, aggregate=False):
    ...
    for entry in m['files']:
        ...
        if entry['url']!=(AGGREGATE_URLS if aggregate else ASSET_URLS)[file] or entry['type']!=kind or not isinstance(entry['sha256'],str) or len(entry['sha256'])!=64 or any(c not in '0123456789abcdef' for c in entry['sha256']):raise ValueError('route, type or hash')
        files.add(file)
...
def prepare(bundle, check=False, vault=False, history=False, aggregate=False):
    ...
    try:
        ...
        manifest=parse_manifest(read(manifest_path,8192),vault,history,aggregate)
        assets={}
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--bundle',type=Path,required=True);p.add_argument('--check',action='store_true');mode=p.add_mutually_exclusive_group();mode.add_argument('--vault',action='store_true');mode.add_argument('--history',action='store_true');mode.add_argument('--aggregate',action='store_true');a=p.parse_args()
    try:prepare(a.bundle,a.check,a.vault,a.history,a.aggregate)
