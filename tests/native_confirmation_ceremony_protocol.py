...
root=Path(__file__).resolve().parents[1];web=root/'experiments/openmls-browser/web';work=Path(tempfile.mkdtemp(prefix='confirmation-protocol-',dir=root/'artifacts'));work.chmod(0o700)
allowed={x:'text/javascript' for x in ('confirmation-ceremony-ui.js','confirmation-ceremony-client.js','custody-ceremony-client.js','candidate-preparation-client.js','peer-preparation-client.js','successor-handoff.js')};allowed.update({'confirmation-ceremony.html':'text/html','confirmation-ceremony.css':'text/css'})
class Handler(BaseHTTPRequestHandler):
 def do_GET(self):
  name=self.path.lstrip('/') or 'confirmation-ceremony.html'
...
try:
 with sync_playwright() as pw:
  ...
  for mode in ('spawn-throw','clone-throw','missing-boot','bad-boot','duplicate-boot','wrong-id','extra-private','bad-key','bad-room','over-memory','failure','lock','broadcast','hide','late-lock'):
   ...
   p.evaluate("""mode=>{
    ...
    window.Worker=class {
     ...
     postMessage(message){window.sentKeys=Object.keys(message.argument).sort();if(mode==='clone-throw')throw Error();
      let data={id:1,ok:true,result:{committed:true,role:'peer',phase:'peer-verified-inactive',peer_verified:true,group_id:'cd'.repeat(16),transcript_revision:2},memory_bytes:1};
   ...
   p.select_option('#identity','alice');p.select_option('#role','peer');p.fill('#database',scope['database']);p.select_option('#action','confirm');p.locator('#request-file').set_input_files({'name':'request.json','mimeType':'application/json','buffer':json.dumps(doc).encode()});p.click('#request-load');expect(p.locator('#request-status')).to_contain_text('요청을 읽었습니다');p.fill('#confirmation',expected);p.fill('#password','generated-password-for-protocol-only-000');p.check('#consent');p.click('#run')
  ...
  p.goto(f'http://127.0.0.1:{server.server_port}/');p.wait_for_selector('#run')
  def parse(raw):return p.evaluate("async raw=>{try{let m=await import('/confirmation-ceremony-client.js');return (await m.request(raw,'alice','family-mls-device-vault-synthetic-protocol','peer')).sha256}catch{return null}}",raw)
  ...
  assert p.evaluate("async d=>{const m=await import('/confirmation-ceremony-client.js'),r=await m.request(JSON.stringify(d),'alice',d.scopes[0].database,'peer');return Object.isFrozen(r.reservation)&&Object.isFrozen(r.reservation.context.peer)}",doc);checks['owned_immutable_reservation_before_private_worker']=True
  def validate(v,role='peer'):
   ...
   return p.evaluate("async ([v,e,role,db])=>{try{const m=await import('/confirmation-ceremony-client.js');return (await m.result(v,role==='peer'?'alice':'bob',db,role,'confirm',e)).state}catch{return null}}",[v,reservation,role,database])
  states={'candidate':['candidate-saved','candidate-recorded','candidate-verified'],'peer':['awaiting-candidate','peer-reply-saved','peer-reply-recorded']}
  for role in states:
   for n,state in enumerate(states[role]):
    verified=n==2 if role=='candidate' else n>=1
    phase='peer-verified-inactive' if verified else 'awaiting-candidate-proof' if n==0 else 'awaiting-peer-proof'
    v={'committed':True,'role':role,'peer_verified':verified,'phase':phase,'transcript_revision':n,'group_id':'cd'*16}
    assert validate(v,role)==state
    for k,value in [('phase','active'),('group_id','ab'*16),('transcript_revision',5),('committed',False),('role','invalid'),('provider','must-reject'),('peer_verified',not verified)]:
     bad=copy.deepcopy(v);bad[k]=value;assert validate(bad,role) is None
    for other_phase in ('awaiting-candidate-proof','awaiting-peer-proof','peer-verified-inactive'):
     for bit in (True,False):
      bad=copy.deepcopy(v);bad.update(phase=other_phase,peer_verified=bit)
      assert (validate(bad,role) is not None)==(other_phase==phase and bit==verified)
  checks['public_confirmation_status_role_revision_verified_matrix_no_activation']=True
