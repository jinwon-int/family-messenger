...
class AssetPreparationTests(unittest.TestCase):
    ...
    def rejected(self):
        with self.assertRaises((OSError, ValueError)):
            m.prepare(self.bundle, vault=self.vault,history=self.history,aggregate=self.aggregate,aggregate_history=self.aggregate_history,successor=self.successor,candidate=self.candidate,peer=self.peer)

    def test_exact_create_reuse_and_check(self):
        with self.assertRaises(ValueError):
            m.prepare(self.bundle, True, vault=self.vault,history=self.history,aggregate=self.aggregate,aggregate_history=self.aggregate_history,successor=self.successor,candidate=self.candidate,peer=self.peer)
        m.prepare(self.bundle, vault=self.vault,history=self.history,aggregate=self.aggregate,aggregate_history=self.aggregate_history,successor=self.successor,candidate=self.candidate,peer=self.peer)
    ...
    def test_symlinked_bundle_parent_denied(self):
        ...
        with self.assertRaises(ValueError):
            m.prepare(link, vault=self.vault,history=self.history,aggregate=self.aggregate,aggregate_history=self.aggregate_history,successor=self.successor,candidate=self.candidate,peer=self.peer)
        self.assertFalse(self.output.exists())
