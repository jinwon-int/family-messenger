...
class AssetPreparationTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bundle.mkdir()
        self.sources = []
        pin = json.loads((m.ROOT/'server/internal/chat/aggregate_bundle.json').read_text()) if self.aggregate else json.loads((m.ROOT/'server/internal/chat/history_bundle.json').read_text()) if self.history else json.loads((m.ROOT/'server/internal/chat/vault_bundle.json').read_text()) if self.vault else json.loads(json.dumps(PIN))
    ...
    def rejected(self):
        with self.assertRaises((OSError, ValueError)):
            m.prepare(self.bundle, vault=self.vault,history=self.history,aggregate=self.aggregate)

    def test_exact_create_reuse_and_check(self):
        with self.assertRaises(ValueError):
            m.prepare(self.bundle, True, vault=self.vault,history=self.history,aggregate=self.aggregate)
        m.prepare(self.bundle, vault=self.vault,history=self.history,aggregate=self.aggregate)
    ...
    def test_symlinked_bundle_parent_denied(self):
        ...
        with self.assertRaises(ValueError):
            m.prepare(link, vault=self.vault,history=self.history,aggregate=self.aggregate)
        self.assertFalse(self.output.exists())
