...
export class AggregateHistoryReader {
 ...
 read(argument,exportSnapshot=false){
  ...
  try{
   if(typeof argument?.password!=='string'||argument.password.length<32||argument.password.length>128||new TextEncoder().encode(JSON.stringify(argument.expected)).length>4096)throw Error('bounds');
