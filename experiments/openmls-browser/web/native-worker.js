...
function controlAAD(q){return enc.encode(JSON.stringify({version:1,room,group_id:q.group_id,client_id:q.client_id,sender_actor:'alice',sender_device:q.device_id,kind:q.kind,expected_revision:q.expected_revision,epoch:q.epoch,target_device:q.target_device}));}
function applyControl(r,method,q){let c;try{c=staged_control_apply(r.crypto,identity,method,method==='peer_update'?bytes(q.payload):new Uint8Array(),peer(r).actor,unhex(peer(r).signing_key),controlAAD(q));r.crypto=c.state();return c.output();}finally{if(c)c.free();}}
const ackID=epoch=>epoch===1?'control-ack':'control-ack-'+epoch;
...
function prepareControl(r){
 ...
 }else if(identity==='bob'&&r.phase==='ack'){if(staged_epoch(r.crypto,identity)!==String(r.epoch))fail();q=wire(r,ackID(r.epoch),'ack',peer(r).device_id,new Uint8Array());}
...
function consume(r,e){
 ...
 else if(q.kind==='ack'){if(phase!=='ack'||q.client_id!==ackID(epoch)||q.device_id!==r.binding.peer_device||q.target_device!==r.binding.creator_device||bytes(q.payload).length)fail();phase='ready';revision++;}
 else if(q.kind==='application'){if(phase!=='ready'||q.target_device!==''||!q.client_id.startsWith('app-')||!bytes(q.payload).length)fail();}
 else if(q.kind==='commit'){if(phase!=='ready'||!q.client_id.startsWith('update-')||q.device_id!==r.binding.creator_device||q.target_device!==r.binding.peer_device||!bytes(q.payload).length)fail();phase='ack';revision++;epoch++;}
 else fail();
 if(e.revision!==revision||e.epoch!==epoch)fail();
 if(self){if(!r.pending||r.pending.retired||!same(r.pending.request,q))fail();if(q.kind==='application'){if(!r.pending.frame)fail();r.messages.push(r.pending.frame);}else if(q.kind==='commit')applyControl(r,'merge_update',q);r.pending=null;}
 ...
 else if(q.kind==='commit'){if(r.pending)fail();applyControl(r,'peer_update',q);}
...
async function dispatch(method,arg){
 ...
 if(method==='update'){
 if(!exact(arg,['id','fault'])||!name(arg.id)||!arg.id.startsWith('update-')||!['','abort-before-write','abort-after-write'].includes(arg.fault))fail();
 const r=await snapshot(),s=await nativeStatus(r),d=await readDirectory(identity,room);
 return tx(d,current=>{staticBinding(current,s);if(identity!=='alice')fail();
 const old=current.receipts.find(e=>e.request.device_id===own(current).device_id&&e.request.client_id===arg.id);if(old){if(old.request.kind!=='commit')fail();return status(current);}
 if(current.pending){if(current.pending.retired||current.pending.request.kind!=='commit'||current.pending.request.client_id!==arg.id)fail();return status(current);}
 if(current.phase!=='ready'||s.phase!=='ready'||current.revision!==s.revision||current.epoch!==s.epoch||current.receipts.length>=32)fail();
 const q=wire(current,arg.id,'commit',peer(current).device_id,new Uint8Array());q.payload=b64(applyControl(current,'update',q));current.pending={request:q,frame:null,retired:false};return status(current);},arg.fault);
 }
 if(method==='prepare'){
...
self.onmessage=({data})=>{if(storage&&data?.method==='lock'){const valid=exact(data,['id','method','argument'])&&Number.isSafeInteger(data.id)&&data.id>0&&data.argument===null;retired=true;storage.close();if(db)db.close();self.postMessage(valid?{id:data.id,ok:true,result:{locked:true},memory_bytes:memory()}:{id:data.id,ok:false,error:'native state rejected',memory_bytes:memory()});self.close();return;}queue=queue.then(async()=>{let id;try{if(!exact(data,['id','method','argument'])||!Number.isSafeInteger(data.id)||data.id<1||typeof data.method!=='string')fail();id=data.id;if(retired)fail();const result=await dispatch(data.method,data.argument);if(memory()>128*1024*1024)fail();if(retired)fail();self.postMessage({id,ok:true,result,memory_bytes:memory()});}catch(_){retired=true;if(storage)storage.close();if(db){db.close();db=undefined;}self.postMessage({id,ok:false,error:'native state rejected',memory_bytes:memory()});}});};
