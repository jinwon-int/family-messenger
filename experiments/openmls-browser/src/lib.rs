...
impl Device {
    ...
    pub fn invite(&mut self, key_package: &[u8]) -> Result<Vec<u8>, JsValue> {
    ...
    pub fn join(&mut self, bytes: &[u8]) -> Result<(), JsValue> {
    ...
    pub fn encrypt(&mut self, bytes: &[u8]) -> Result<Vec<u8>, JsValue> {
        bounded(bytes, 16384)?;
    ...
    pub fn decrypt(&mut self, bytes: &[u8]) -> Result<Vec<u8>, JsValue> {
        bounded(bytes, MAX_WIRE)?;
        let message = MlsMessageIn::tls_deserialize_exact_bytes(bytes).map_err(rejected)?
    ...
    pub fn remove_invitee(&mut self) -> Result<Vec<u8>, JsValue> {
        let group = self.group.as_mut().ok_or_else(|| rejected(()))?;
    ...
    pub fn apply_commit(&mut self, bytes: &[u8]) -> Result<(), JsValue> {
        bounded(bytes, MAX_WIRE)?;
        let message = MlsMessageIn::tls_deserialize_exact_bytes(bytes).map_err(rejected)?
            .try_into_protocol_message().map_err(rejected)?;
        let group = self.group.as_mut().ok_or_else(|| rejected(()))?;
